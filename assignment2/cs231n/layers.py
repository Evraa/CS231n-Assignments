...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1 / N) * gamma * inverse_var * ( (N * dout) - np.sum(dout, axis=0)\
        - (x_minus_mean * np.square(inverse_var) * np.sum(dout * x_minus_mean, axis=0)))
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        #######################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        mask =  (np.random.rand(*x.shape) < keep_prob) / p
        out = x * mask
...
def max_pool_backward_naive(dout, cache):
    ...
    for ex in range(N):
        dout_row = dout[ex].reshape(C, outH*outW)
        neuron = 0
        for i in range(0, H-PH+1, stride):
            for j in range(0, W-PW+1, stride):
                pool_region = x[ex,:,i:i+PH,j:j+PW].reshape(C,PH*PW)
                ...
                dx[ex,:,i:i+PH,j:j+PW] += dmax_pool.reshape(C,PH,PW)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    #Don't feel like doin it right now
    pass 
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
